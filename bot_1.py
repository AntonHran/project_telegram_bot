...
from functions import get_api_key, translate, form_quote, form_article, form_weather, form_news, form_books, form_trends
...
# Commands
SELECTING_ACTION, LANG, DONE, BOOKS, CHANGE_LANGUAGE, \
    NEWS, ARTICLE_TIME, MOTIVATION_TIME, COUNTRY, CATEGORY, CITY_WEATHER, TRENDS = range(12)
menu_categories = []
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_lang = ReplyKeyboardMarkup(button_lang, resize_keyboard=True, one_time_keyboard=True)
    await update.message.reply_text('Please choose a language you would like to speak with me and receive messages. / '
...
async def lang_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f'lang: {lang}')
    await update.message.reply_text('You have chosen English. LONG LIVE the KING!' if lang == 'eng' else
                                    'Ви обрали українську мову. СЛАВА УКРАЇНІ!',
                                    reply_markup=telegram.ReplyKeyboardRemove())
    buttons_menu = [[telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['motivation'],
                                                   callback_data=str(MOTIVATION_TIME)),
                     telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['weather'],
                                                   callback_data=str(CITY_WEATHER))],

                    [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['twitter_trends'],
                                                   callback_data=str(TRENDS)),
                     telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['wiki'],
                                                   callback_data=str(ARTICLE_TIME))],

                    [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['news'],
                                                   callback_data=str(NEWS)),
                     telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['books'],
                                                   callback_data=str(BOOKS))],

                    [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['change_language'],
                                                   callback_data=str(CHANGE_LANGUAGE))],

                    [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['done'],
                                                   callback_data=str(DONE))], ]
    reply = telegram.InlineKeyboardMarkup(inline_keyboard=buttons_menu)
    await update.message.reply_text(text=lang_dict[lang]['phr']['set'], reply_markup=reply)
...
async def settings_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data.update({chat_id_: dict(name=query.from_user.name,
                                article=[],
                                books=[],
                                motivation=[],
                                ...
                                weather=[])})
    if menu == str(DONE):
        await query.message.reply_text(text=f"{lang_dict[lang]['phr']['cat_chosen']} "
                                            f"{', '.join([lang_dict[lang]['keys'][keys[cat]] for cat in menu_categories])}")

        button_cont = [[KeyboardButton(text=lang_dict[lang]['keys']['cont'])]]
        reply = ReplyKeyboardMarkup(button_cont, resize_keyboard=True, one_time_keyboard=True)
        await query.message.reply_text(text=lang_dict[lang]['phr']['continue'][0],
                                       reply_markup=reply)
...
async def next_state_func(update, context) -> int:
    ...
    if str(MOTIVATION_TIME) in context.user_data.keys() and context.user_data[str(MOTIVATION_TIME)][1] != 'done':
    ...
    elif str(ARTICLE_TIME) in context.user_data.keys() and context.user_data[str(ARTICLE_TIME)][1] != 'done':
        await update.message.reply_text(text=lang_dict[lang]['phr']['wiki'])
        context.user_data[str(ARTICLE_TIME)][1] = 'done'
        return ARTICLE_TIME
    elif str(CITY_WEATHER) in context.user_data.keys() and context.user_data[str(CITY_WEATHER)][1] != 'done':
        await update.message.reply_text(text=lang_dict[lang]['phr']['weather'])
        context.user_data[str(CITY_WEATHER)][1] = 'done'
        return CITY_WEATHER
    elif str(TRENDS) in context.user_data.keys() and context.user_data[str(TRENDS)][1] != 'done':
        await update.message.reply_text(text=lang_dict[lang]['phr']['twitter_trends'])
        context.user_data[str(TRENDS)][1] = 'done'
        return TRENDS
    elif str(BOOKS) in context.user_data.keys() and context.user_data[str(BOOKS)][1] != 'done':
        await update.message.reply_text(text=lang_dict[lang]['phr']['books'])
        context.user_data[str(BOOKS)][1] = 'done'
        return BOOKS
    elif str(CATEGORY) in context.user_data.keys() and context.user_data[str(CATEGORY)][1] != 'done':
        countries.clear()
        categories.clear()
        buttons_category = [[telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['business'],
                                                           callback_data='business')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['entert'],
                                                           callback_data='entertainment')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['general'],
                                                           callback_data='general')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['health'],
                                                           callback_data='health')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['science'],
                                                           callback_data='science')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['sports'],
                                                           callback_data='sports')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['tech'],
                                                           callback_data='technology')],
                            [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['dn_c'],
                                                           callback_data='done')], ]
        reply = telegram.InlineKeyboardMarkup(inline_keyboard=buttons_category)
        await update.message.reply_text(text=lang_dict[lang]['phr']['category'], reply_markup=reply)
...
async def continue_(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(text=lang_dict[lang]['phr']['continue'][1],
                                    reply_markup=telegram.ReplyKeyboardRemove())
...
async def article_time(update, context):
    ...
    if ans and ans != 'skip':
        data[chat_id_]['article'].append(datetime.datetime.strptime(ans.strip(), '%H:%M').time())
    elif ans == 'skip':
        data[chat_id_]['article'].append(datetime.datetime.strptime('9:00', '%H:%M').time())
    next_state = await next_state_func(update, context)
    return next_state
...
async def set_time_mot(update, context):
    ...
    if ans and ans != 'skip':
        data[chat_id_]['motivation'].append(datetime.datetime.strptime(ans, '%H:%M').time())
    elif ans == 'skip':
        data[chat_id_]['motivation'].append(datetime.datetime.strptime('9:00', '%H:%M').time())
    next_state = await next_state_func(update, context)
    return next_state
...
async def category_chosen(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if category == 'done':
        buttons_country = [[telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['ukr'], callback_data='ua')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['usa'], callback_data='us')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['ger'], callback_data='gr')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['gb'], callback_data='gb')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['fr'], callback_data='fr')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['itl'], callback_data='it')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['pld'], callback_data='pl')],
                           [telegram.InlineKeyboardButton(text=lang_dict[lang]['keys']['dn_c'], callback_data='done')]]
        await query.message.reply_text(text=lang_dict[lang]['phr']['country'],
    ...
    for key, value in data[chat_id_].items():
        if key in func and func[key][1]:
            ...
            if type(func[key][1]) is datetime.time or type(func[key][1]) is str:
                ...
                await query.answer()
                if mes:
                    [await query.message.reply_text(el) if lang == 'eng' else await query.message.reply_text(
                        translate(el)) for el in mes]
                else:
                ...
                mes = f()
            # print(mes)
            await update.message.reply_text(mes) if lang == 'eng' else await update.message.reply_text(translate(mes))
...
def get_access(chat_id):
    func = {'article': (form_article, data[chat_id]['article'][1] if data[chat_id]['article'] else None),
            'books': (form_books, data[chat_id]['books'][1] if data[chat_id]['books'] else None),
            'motivation': (form_quote, data[chat_id]['motivation'][1] if data[chat_id]['motivation'] else None),
            'news': (form_news,
                     [data[chat_id]['news']['category'], data[chat_id]['news']['country']] if data[chat_id]['news'][
                         'category'] else None),
            'trends': (form_trends, data[chat_id]['trends'][0] if data[chat_id]['trends'] else None),
            'weather': (form_weather, data[chat_id]['weather'][1] if data[chat_id]['weather'] else None)}
    return func
...
def main():
    ...
    conv_handler_settings = ConversationHandler(
        ...
        states={ARTICLE_TIME: [MessageHandler(filters.TEXT, article_time)],
